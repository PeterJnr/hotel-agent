...
export function AdminLayout() {
  ...
  return <div className="admin-shell"><aside className={open ? "admin-sidebar open" : "admin-sidebar"}><div className="admin-logo"><Link to="/"><span className="brand-mark">AS</span><span><strong>Apex Solacii</strong><small>Hotel administration</small></span></Link><button onClick={() => setOpen(false)} aria-label="Close menu"><X /></button></div><nav>{visibleLinks.map(({ to, label, icon: Icon, end }) => <NavLink end={end} to={to} key={to} onClick={() => setOpen(false)}><Icon /><span>{label}</span><ChevronRight /></NavLink>)}</nav><div className="admin-profile"><span>{user.firstName[0]}{user.lastName[0]}</span><div><strong>{user.firstName} {user.lastName}</strong><small>{user.roles.map((role) => role.replaceAll("_", " ")).join(" · ")}</small></div><button onClick={logout} title="Sign out"><LogOut /></button></div></aside><div className="admin-workspace"><header className="admin-topbar"><button onClick={() => setOpen(true)} aria-label="Open menu"><Menu /></button><div className="layout-actions"><ThemeToggle /><div className="live-operations"><BarChart3 /><span>Live operations</span><i /></div></div></header><Outlet /></div></div>;
}
