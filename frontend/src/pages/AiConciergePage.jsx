...
const titleFor = (conversation) => conversation.title || "Solacii AI conversation";
...
function ConfirmationCard({ action, busy, onConfirm, onCancel }) {
  const confirmationLabel = action.name === "initialize_payment" ? "Confirm payment" : "Confirm";
  return <div className="ai-confirmation"><span><Sparkles /> Your confirmation is required</span><h3>{readable(action.name)}</h3><dl>{Object.entries(action.arguments || {}).map(([key, value]) => <div key={key}><dt>{readable(key)}</dt><dd>{typeof value === "object" ? JSON.stringify(value) : String(value)}</dd></div>)}</dl><small>{action.name === "initialize_payment" ? "Confirm to create your secure Paystack checkout link. Card details are entered only on Paystack." : "Nothing will be changed until you confirm."}</small><div><button className="button dark" disabled={busy} onClick={onConfirm}><Check /> {confirmationLabel}</button><button className="button outline" disabled={busy} onClick={onCancel}><X /> Cancel</button></div></div>;
}
...
export function AiConciergePage() {
  ...
  const pendingAction = proposedAction || conversation?.pendingActions?.[0] || null;
  ...
  const removePendingAction = (actionId) => {
    ...
    queryClient.setQueryData(["ai-conversation", conversationId], (current) => {
      if (!current) return current;
      const detail = current.data || current;
      const updated = { ...detail, pendingActions: detail.pendingActions?.filter((action) => action.id !== actionId) || [] };
      return current.data ? { ...current, data: updated } : updated;
    });
  };
  const confirm = useMutation({
    mutationFn: (action) => api.post(`/api/ai/actions/${action.id}/confirm`),
    onSuccess: ({ data }, action) => {
      removePendingAction(action.id);
      setActionNotice(data.message || "Your request has been completed.");
      refresh();
      if (action.name === "initialize_payment" && data.data?.authorizationUrl) window.location.assign(data.data.authorizationUrl);
    },
  });
  const cancel = useMutation({ mutationFn: (action) => api.delete(`/api/ai/actions/${action.id}`), onSuccess: ({ data }, action) => { removePendingAction(action.id); setActionNotice(data.message || "That action was cancelled."); refresh(); } });
  ...
  const selectConversation = (id) => { setConversationId(id); setLocalMessages([]); setProposedAction(null); setActionNotice(null); };

  return <div className="concierge-screen"><aside className="ai-history"><div><span className="eyebrow">Private concierge</span><h1>Ask Solacii AI</h1></div><button className="new-chat" onClick={() => selectConversation(null)}><MessageSquarePlus /> New conversation</button><nav>{conversations.map((item) => <button className={item.id === conversationId ? "active" : ""} onClick={() => selectConversation(item.id)} key={item.id}><span>{titleFor(item)}</span><small>{new Intl.DateTimeFormat("en", { month: "short", day: "numeric" }).format(new Date(item.lastMessageAt))}</small></button>)}</nav></aside><main className="ai-chat"><header><div><Bot /><span><strong>Solacii AI</strong><small>AI guest concierge · actions require approval</small></span></div>{conversationId && <button onClick={() => archive.mutate()} disabled={archive.isPending}><Archive /> Archive</button>}</header><div className="ai-thread">{!conversationId && !localMessages.length && <div className="ai-intro"><div><Sparkles /></div><span className="eyebrow">Always at your service</span><h2>What may I arrange for you?</h2><p>Explore rooms, check availability, review your stays, or prepare a reservation—all through conversation.</p><div>{prompts.map((prompt) => <button key={prompt} onClick={() => send(prompt)}>{prompt}</button>)}</div></div>}{loadingConversation && <div className="portal-loading">Reopening your conversation…</div>}{messages.map((message) => <article className={`ai-message ${message.role.toLowerCase()}`} key={message.id}><span>{message.role === "ASSISTANT" ? <Bot /> : "You"}</span><MessageText>{message.content}</MessageText></article>)}{chat.isPending && <article className="ai-message assistant typing"><span><Bot /></span><p>Solacii AI is considering your request<span>···</span></p></article>}{pendingAction && <ConfirmationCard action={pendingAction} busy={confirm.isPending || cancel.isPending} onConfirm={() => confirm.mutate(pendingAction)} onCancel={() => cancel.mutate(pendingAction)} />}{actionNotice && <div className="ai-action-notice"><Check /> {actionNotice}</div>}{(chat.error || confirm.error || cancel.error || archive.error) && <div className="form-error">{(chat.error || confirm.error || cancel.error || archive.error).message}</div>}<div ref={bottomRef} /></div><form className="ai-composer" onSubmit={(event) => { event.preventDefault(); send(); }}><textarea value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); send(); } }} placeholder={pendingAction ? "Confirm or cancel the proposed action first" : "Ask Solacii AI anything about your stay…"} disabled={Boolean(pendingAction)} maxLength="4000" /><button disabled={!draft.trim() || chat.isPending || Boolean(pendingAction)} aria-label="Send message"><Send /></button><small>Solacii AI can make mistakes. Review dates, guests, prices, and room details before confirming.</small></form></main></div>;
}
